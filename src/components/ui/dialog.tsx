...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "nfixed nleft-[50%] ntop-[50%] nz-50 ngrid   ntranslate-x-[-50%] ntranslate-y-[-50%] ngap-4 nborder nbg-background np-6 nshadow-lg nduration-200 data-[state=open]:nanimate-in data-[state=closed]:nanimate-out data-[state=closed]:nfade-out-0 data-[state=open]:nfade-in-0 data-[state=closed]:nzoom-out-95 data-[state=open]:nzoom-in-95 data-[state=closed]:nslide-out-to-left-1/2 data-[state=closed]:nslide-out-to-top-[48%] data-[state=open]:nslide-in-from-left-1/2 data-[state=open]:nslide-in-from-top-[48%] sm:nrounded-lg",
        className
      )}
      {...props}
    >
      {children}
      <DialogPrimitive.Close className="nabsolute nright-4 ntop-4 nrounded-sm nopacity-70 nring-offset-background ntransition-opacity hover:nopacity-100 focus:noutline-none focus:nring-2 focus:nring-ring focus:nring-offset-2 disabled:npointer-events-none data-[state=open]:nbg-accent data-[state=open]:ntext-muted-foreground">
...
const DialogHeader = ({
  ...
  <div
    className={cn(
      "nflex nflex-col nspace-y-1.5 ntext-center sm:ntext-left",
      className
    )}
    {...props}
  />
